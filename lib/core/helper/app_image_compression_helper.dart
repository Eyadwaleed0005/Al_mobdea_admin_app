...
class AppImageCompressionHelper {
  ...
  static const int targetSizeInBytes =
      targetSizeInKilobytes * 1024;
  ...
  static Future<AppImageCompressionResult> compressForFirebase({
    ...
    for (final attempt in _compressionAttempts) {
      ...
      if (compressedBytes.lengthInBytes <
          smallestBytes.lengthInBytes) {
  ...
  static String _normalizeOriginalFileName(
  ...
  static String _buildCompressedFileName(
    String originalFileName,
  ) {
    final normalizedName = originalFileName.trim();
