...
class ExamDeletionService {
  const ExamDeletionService({
    ...
    required ExamsDataValidator examsDataValidator,
    required ExamEditingPolicy examEditingPolicy,
    required ExamQuestionImageService examQuestionImageService,
  }) : _firebaseFirestore = firebaseFirestore,
       _examsDataValidator = examsDataValidator,
       _examEditingPolicy = examEditingPolicy,
       _examQuestionImageService = examQuestionImageService;
  ...
  CollectionReference<Map<String, dynamic>> get _results {
    return _firebaseFirestore.collection(
      FirestoreCollections.examResults,
    );
  }
  ...
  Future<void> deleteExam({required String examId}) async {
    final String normalizedExamId = _examsDataValidator
        .validateId(examId);

    final DocumentReference<Map<String, dynamic>> examReference =
        _exams.doc(normalizedExamId);
  ...
  Future<void> deleteQuestion({
    ...
    final String normalizedQuestionId = _examsDataValidator
    ...
    questionReference = _questions.doc(normalizedQuestionId);
    ...
    final String? imageStoragePath = await _firebaseFirestore
        .runTransaction<String?>((
          ...
          examSnapshot = await transaction.get(examReference);
          ...
          final ExamModel exam = ExamModel.fromFirestore(
            examSnapshot,
          );
          ...
          final DocumentSnapshot<Map<String, dynamic>>
          questionSnapshot = await transaction.get(
            questionReference,
          );

          if (!questionSnapshot.exists ||
              questionSnapshot.data() == null) {
            return null;
          }

          final ExamQuestionModel question =
              ExamQuestionModel.fromFirestore(questionSnapshot);
          ...
          final int updatedTotalScore =
              exam.totalScore - question.degree;
  ...
  Future<void> _deleteExamAttemptAnswers({
    required String examId,
  }) {
  ...
  Future<void> _deleteExamQuestions({
    required String examId,
  }) async {
    while (true) {
      final QuerySnapshot<Map<String, dynamic>>
      questionsSnapshot = await _questions
          .where(FirestoreFields.examId, isEqualTo: examId)
          .limit(_deleteBatchSize)
          .get(const GetOptions(source: Source.server));
      ...
      for (final QueryDocumentSnapshot<Map<String, dynamic>>
        ...
        final ExamQuestionModel question =
            ExamQuestionModel.fromFirestore(questionDocument);
