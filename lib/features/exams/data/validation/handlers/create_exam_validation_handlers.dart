...
import 'package:al_mobdea_admin/features/exams/data/validation/core/validation_handler.dart';
...
class CreateExamGradeIdHandler
  ...
  CreateExamGradeIdHandler({
    required ExamsDataValidator validator,
...
class CreateExamQuestionsLimitHandler
  ...
  void validate(CreateExamValidationData data) {
    if (data.questions.length >
        ExamsDataValidator.maxQuestionsPerExam) {
      FirebaseErrorHandler.throwFirestoreCode(
        'invalid-argument',
      );
    }
...
class CreateExamQuestionsPayloadHandler
    extends ValidationHandler<CreateExamValidationData> {
  CreateExamQuestionsPayloadHandler({
    required ExamsDataValidator validator,
  }) : _validator = validator;
...
class CreateExamUniqueQuestionIdsHandler
    extends ValidationHandler<CreateExamValidationData> {
  CreateExamUniqueQuestionIdsHandler({
    required ExamsDataValidator validator,
  }) : _validator = validator;
  ...
  void validate(CreateExamValidationData data) {
    ...
    for (final ExamQuestionModel question in data.questions) {
      final String questionId = _validator.validateId(
        question.questionId,
      );
...
class CreateExamTotalScoreHandler
  ...
  @override
  void validate(CreateExamValidationData data) {
    final int calculatedTotalScore = data.questions.fold<int>(
      0,
      (int total, ExamQuestionModel question) {
...
class CreateExamImageKeysHandler
  ...
  void validate(CreateExamValidationData data) {
    ...
    for (final String imageQuestionId
        in data.questionImages.keys) {
      final String normalizedId = _validator.validateId(
        imageQuestionId,
      );
...
class CreateExamImageReferencesHandler
    extends ValidationHandler<CreateExamValidationData> {
  CreateExamImageReferencesHandler({
    required ExamsDataValidator validator,
  }) : _validator = validator;
  ...
  void validate(CreateExamValidationData data) {
    ...
    for (final ExamQuestionModel question in data.questions) {
      ...
      if (question.hasImage &&
          !imageQuestionIds.contains(questionId)) {
        FirebaseErrorHandler.throwFirestoreCode(
          'invalid-argument',
        );
      }
...
class CreateExamImageFilesHandler
  ...
  @override
  void validate(CreateExamValidationData data) {
    for (final ExamQuestionImageFile image
        in data.questionImages.values) {
