...
import 'package:al_mobdea_admin/features/students/domain/helper/date_validation_helper.dart';
...
class UpdateStudentSubscriptionUseCase {
  ...
  Future<Either<AppErrorModel, StudentEntity>> call({
    ...
    final isValidDateRange = DateValidationHelper.isValidDateRange(
    ...
    final isEndDateInFuture = DateValidationHelper.isEndDateInFuture(
      endAt: subscriptionEndAt,
    );
    ...
    // A new subscription or renewal re-activates the student account.
    final authResult = await _studentAuthRepository.updateStudentAccountStatus(
      studentId: student.studentId,
      isActive: true,
    );

    return authResult.fold<Future<Either<AppErrorModel, StudentEntity>>>(
      (error) async {
      ...
      (_) async {
        ...
        final firestoreResult = await _studentsRepository.updateStudent(
          student: updatedStudent,
        );

        return firestoreResult
            .fold<Future<Either<AppErrorModel, StudentEntity>>>(
          (error) async {
            // Rollback the auth status if the Firestore update fails.
          ...
          (_) async {
            return Right<AppErrorModel, StudentEntity>(updatedStudent);
          },
        );
      },
    );
